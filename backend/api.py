...
def get_films(nr=1):
    Table = []
    Films = app.models.Film.query.filter(
        app.models.Film.ID.between((int(nr) - 1) * 10, int(nr) * 10 - 1)
    )
    for Data in Films:
        Table.append(
            {
...
def get_serials(nr=1):
    Table = []
    Serials = app.models.Serial.query.filter(
        app.models.Serial.ID.between((int(nr) - 1) * 10, int(nr) * 10 - 1)
    )
    for Data in Serials:
        Table.append(
            {
...
def search(name):
    if name:
        try:
            ...
            Films = app.models.Film.query.filter(
                app.models.Film.Title.like("%" + name + "%")
            ).all()
            Serials = app.models.Serial.query.filter(
                app.models.Serial.Title.like("%" + name + "%")
            ).all()
            ...
            Table.append(
                {
                    ...
                    "User": app.models.User.query.filter_by(ID=Comment.User_ID)
                    .first()
                    .Username,
        ...
        if nr:
            ...
            if type == "film":
                Comment = app.models.Comment(
                    User_ID=app.models.User.query.filter_by(Username=current_user)
                    .first()
                    .ID,
                    Comment=comment,
                    Film_ID=nr,
...
def get_film_rating(type=False, nr=False):
    ...
    if nr and type and current_user:
        ...
        if type == "film":
            Rating = app.models.Rating.query.filter_by(
                Film_ID=nr,
                User_ID=app.models.User.query.filter_by(Username=current_user)
                .first()
                .ID,
...
def get_all_film_rating():
    ...
    if current_user:
        ...
        Ratings = app.models.Rating.query.filter_by(
                ...
                Table.append(
                    {
                        ...
                        "DataName": app.models.Film.query.filter_by(ID=Rating.Film_ID)
                        .first()
                        .Title,
                        "Rating": Rating.Rate,
                    }
                ...
                Table.append(
                    {
                        ...
                        "DataName": app.models.Serial.query.filter_by(
                            ID=Rating.Serial_ID
                        )
                        .first()
                        .Title,
...
def add_film_rating(nr=False):
    ...
    try:
        if nr:
            ...
            if type == "film":
                if app.models.Rating.query.filter_by(
                    Film_ID=nr,
                    User_ID=app.models.User.query.filter_by(Username=current_user)
                    .first()
                    .ID,
